...
local function lock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'yes' then
    return 'درحال حاظر نام گروه قفل است'
  else
...
local function unlock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'no' then
    return 'درحال حاظر نام گروه قفل نیست'
  else
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'درحال حاظر خروج اعضای گروه قفل است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'خروج اعضای گروه قفل شد'
end

local function unlock_group_membermod(msg, data, target)
  local group_member_lock = data[tostring(target)]['settings']['lock_member']
  if group_member_lock == 'no' then
    return 'درحال حاظر خروج اعضای گروه قفل نیست'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
  return 'قفل خروج اعضای گروه باز شد'
  end
end
...
local function show_group_settingsmod(msg, data, target)
    ...
    local text = "تنظیمات گروه:\nنام گروه: "..settings.lock_name.."\nقفل عکس گروه: "..settings.lock_photo.."\nخروج اعضای گروه "..settings.lock_member.."\nحساسیت به اسپم"..NUM_MSG_MAX
    return text
end
...
local function run(msg, matches)
  if msg.to.type ~= 'chat' then
    ...
    if tonumber(matches[3]) == tonumber(our_id) then return false end
      ...
      local user_id = matches[3]
      if tonumber(matches[3]) == tonumber(msg.from.id) then 
        return "شما نمی توانید خودتان را از حالت بن آزاد کنید"
      end
      local hash =  'banned:'..matches[1]
      redis:srem(hash, user_id)
      local name = user_print_name(msg.from)
      savelog(matches[1], name.." ["..msg.from.id.."] unbanned user ".. matches[3])
      return 'کاربر '..user_id..' از حالت بن آزاد شد'
    end
    ...
    if matches[2] == "setflood" then
      ...
      if tonumber(matches[3]) < 5 or tonumber(matches[3]) > 20 then
        return "عدد وارد شده ممکن نیست از 5 تا 20 میتوانید وارد کنید"
      end
      ...
      return 'تعداد حساسیت به اسپم تغییر یافت به '..matches[3]
    end
    ...
    if matches[2] == 'get' then 
      if matches[3] == 'link' then
        ...
        if not group_link then 
          return "ابتدا لینک گروه را ایجاد کنید"
        end
        local name = user_print_name(msg.from)
    ...
    if matches[1] == 'changeabout' and matches[2] and is_owner2(msg.from.id, matches[2]) then
      ...
      local name = user_print_name(msg.from)
      savelog(matches[2], name.." ["..msg.from.id.."] توضیحات گروه تغییر یافت به✔️ ["..matches[3].."]")
    ...
    if matches[1] == 'changerules' and is_owner2(msg.from.id, matches[2]) then
      ...
      local target = matches[2]
      local name = user_print_name(msg.from)
      savelog(matches[2], name.." ["..msg.from.id.."] قوانین گروه تغییر یافت به✔️ ["..matches[3].."]")
